...
type Timeframe = "1m" | "5m" | "15m" | "30m" | "1H" | "4H" | "6h" | "1D";
...
// ─── Bollinger Bands ─────────────────────────────────────────────────────────
function calcBB(values: number[], period = 20, mult = 2): (BollingerBand | null)[] {
...
const fmtTime = (ts: number, tf: Timeframe): string => {
  const d = new Date(ts);
...
export default function ProTradingChart({candles}: Props) {
  ...
  const [activeIndicators, setActiveIndicators] = useState<Set<string>>(
    new Set(["ema20", "ema50", "volume"])
  );
  const [showIndicatorsPanel, setShowIndicatorsPanel] = useState(false);
  const [showOrderBook, setShowOrderBook] = useState(true);
  const [orderBook, setOrderBook] = useState<OrderBook>(() => generateOrderBook(43250));
  ...
  const [dims, setDims] = useState<Dims>({ w: 900, h: 520 });
  const [isDragging, setIsDragging] = useState(false);
  const [dragX, setDragX] = useState<number | null>(null);

  const PAD = { l: 8, r: 72, t: 38, b: 32 } as const;
    ...
    const id = setInterval(() => {
      setCandles(prev => {
        ...
        if (Math.random() < 0.03) {
          return [
            ...next,
            { time: Date.now(), open: newClose, high: newClose, low: newClose, close: newClose, volume: Math.random() * 50 },
          ];
        }
        return next;
      });
      setOrderBook(generateOrderBook(candles[candles.length - 1]?.close ?? 43250));
    }, 800);
...
*/
  ...
  const toY = (v: number): number => PAD.t + mainH - ((v - minP) / range) * mainH;
  const toX = (i: number): number => PAD.l + i * (candleW + 2) + candleW / 2;
  ...
  const handleMouseMove = useCallback(
    ...
    [isDragging, dragX, candleW, slice, minP, range, mainH, candles.length, visibleCount, PAD.t, toX]
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Chart type */}
        <div style={{ display: "flex", gap: 2 }}>
          {([ ["candles", "🕯"], ["line", "📈"] ] as [ChartType, string][]).map(([type, icon]) => (
          ...
          {INDICATORS.map(ind => (
            <button key={ind.id} onClick={() => toggleIndicator(ind.id)} style={{
              ...
              <span style={{
                ...
                background: activeIndicators.has(ind.id) ? ind.color : "#1a2a3a",
      ...
      {hovered && (
        <div style={{
          ...
          <span>C <span style={{ color: hovered.candle.close >= hovered.candle.open ? "#00e676" : "#ff4c6a" }}>
            {fmt(hovered.candle.close)}
          </span></span>
          ...
          <span style={{ color: hovered.candle.close >= hovered.candle.open ? "#00e67677" : "#ff4c6a77" }}>
            ...
            {fmt(Math.abs(hovered.candle.close - hovered.candle.open))} (
            {fmt(Math.abs((hovered.candle.close - hovered.candle.open) / hovered.candle.open * 100), 2)}%)
          </span>
        </div>
      )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div ref={containerRef} style={{ flex: 1, position: "relative", minWidth: 0 }}>
          <svg
            ...
            onMouseDown={e => {
              setIsDragging(true);
              setDragX(e.clientX - (svgRef.current?.getBoundingClientRect().left ?? 0));
            }}
          ...
          >
            ...
            {gridPrices.map((p, i) => {
              ...
              return (
                <g key={i}>
                  <line x1={PAD.l} y1={y} x2={dims.w - PAD.r} y2={y} stroke="#0d1824" strokeWidth="1" />
                  <text x={dims.w - PAD.r + 4} y={y + 3.5} fontSize="9" fill="#2e4a6a" fontFamily="inherit" textAnchor="start">
                    {fmt(p, 0)}
                  </text>
                </g>
              );
            ...
            {([
              ...
              { v: priceLow24, color: "#ff4c6a33", label: "24H L" },
            ] as { v: number; color: string; label: string }[]).map(({ v, color, label }) => {
              const y = toY(v);
              if (y < PAD.t || y > PAD.t + mainH) return null;
              return (
                <g key={label}>
                  <line x1={PAD.l} y1={y} x2={dims.w - PAD.r} y2={y} stroke={color} strokeWidth="1" strokeDasharray="4,4" />
            ...
            {activeIndicators.has("bb") && (
              <g clipPath="url(#chartArea)">
                <path
                  d={bbv.filter((b): b is BollingerBand => b !== null)
                    .map((b, i) => `${i === 0 ? "M" : "L"} ${toX(bbv.findIndex((v, j) => j >= i && v !== null))} ${toY(b.upper)}`)
                    .join(" ")}
                  fill="none" stroke="#00d08444" strokeWidth="1"
                />
                <path
                  d={bbv.filter((b): b is BollingerBand => b !== null)
                    .map((b, i) => `${i === 0 ? "M" : "L"} ${toX(bbv.indexOf(b))} ${toY(b.lower)}`)
                    .join(" ")}
                  fill="none" stroke="#00d08444" strokeWidth="1"
                />
                <path
                  d={bbv.filter((b): b is BollingerBand => b !== null)
                    .map((b, i) => `${i === 0 ? "M" : "L"} ${toX(bbv.indexOf(b))} ${toY(b.mid)}`)
                    .join(" ")}
            ...
            {crosshair && (
              <g opacity="0.6">
                ...
                <line x1={PAD.l} y1={crosshair.y} x2={dims.w - PAD.r} y2={crosshair.y}
                  stroke="#4da6ff" strokeWidth="0.8" strokeDasharray="3,3" />
                <rect x={dims.w - PAD.r} y={crosshair.y - 8} width={PAD.r} height={16} fill="#1a3a5c" rx="2" />
                <text x={dims.w - PAD.r + 4} y={crosshair.y + 4} fontSize="9" fill="#4da6ff" fontFamily="inherit">
            ...
            {hasRSI && (() => {
              ...
              const rsiToY = (v: number) => py0 + subH - ((v - rsiMin) / (rsiMax - rsiMin)) * subH;
              return (
                <g>
                  ...
                  <path
                    d={rsiV
                      .map((v, i) => v == null ? null : `${i === 0 || rsiV[i - 1] == null ? "M" : "L"} ${toX(i)} ${rsiToY(v as number)}`)
                  ...
                  {crosshair && (() => {
                    ...
                    const rvNum = rv as number;
                    return (
                      <g>
                        <circle cx={toX(ci2)} cy={rsiToY(rvNum)} r="3" fill="#ff6b6b" />
                        <rect x={dims.w - PAD.r} y={rsiToY(rvNum) - 8} width={PAD.r} height={16} fill="#1a0a0a" rx="2" />
                        <text x={dims.w - PAD.r + 4} y={rsiToY(rvNum) + 4} fontSize="9" fill="#ff6b6b" fontFamily="inherit">
                          {fmt(rvNum, 1)}
                        </text>
                      </g>
                    );
            ...
            {hasMACD && (() => {
              ...
              const allVals = macdV.hist.filter((v): v is number => v != null);
              const macdMin = Math.min(...allVals);
              const macdMax = Math.max(...allVals);
              const macdRange = macdMax - macdMin || 1;
              const mToY = (v: number) => py0 + subH - ((v - macdMin) / macdRange) * subH;
              const zeroY = mToY(0);
              return (
                <g>
                  <rect x={PAD.l} y={py0} width={W} height={subH} fill="#04080f" />
            ...
            {/* Chart border */}
            <rect x={PAD.l} y={PAD.t} width={W} height={mainH} fill="none" stroke="#0d1824" strokeWidth="1" />
              ...
              {([
                ...
                .map((item, idx) => (
                  <g key={item.label} transform={`translate(${PAD.l + 4 + idx * 70}, ${PAD.t + 4})`}>
                    ...
                    <text x="17" y="9" fontSize="9" fill={item.color} fontFamily="inherit" opacity="0.8">
                      {item.label}
                    </text>
                  </g>
                ))}
          ...
          <div style={{
            ...
            Ctrl+Scroll to zoom • Drag to scroll • Hover candle for details
          </div>
        </div>
